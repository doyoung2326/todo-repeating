...
성격 관리. 설정 화면에는 아직 계정에 관한 것만 있다.

`TodoItem`은 `onEdit`을 받을 자리를 이미 갖고 있다 — 넘기지 않으면 ⋯ 메뉴에서 "수정"
줄이 빠진다. 폼이 붙는 날 그 자리에 끼우면 된다.

오늘 화면의 **진행률은 읽기 전용이다.** 웹은 `<input type=range>`로 그 자리에서
고치는데, RN에는 그런 기본 부품이 없어 슬라이더를 따로 들여야 한다.
