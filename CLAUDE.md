...
- **로그인 관문은 `src/app/_layout.tsx`에서 통째로 가른다.** 라우터로 가르지 않는 이유 —
  ...
  가르는 것은 **`Stack.Protected`의 `guard`**다. 조건부 `<Stack.Screen>`으로는 갈리지 않는다 —
  그것은 "이 화면을 보여줘라"가 아니라 "이 경로의 설정은 이렇다"는 선언이라, 빼도 그 경로는
  그대로 열린다. 한때 그렇게 돼 있어서 로그아웃 상태로 `/`에 들어가면 탭이 떴고, 로그인에
  성공해도 화면이 넘어가지 않았다 — 로그인 화면은 스스로 이동하지 않고 이 갈래를 기다린다.
- **`Alert`을 쓰지 않는다.** `react-native-web`의 `Alert`는 본문이 빈 함수라 `npm run web`으로
  개발하는 동안 확인창이 아예 뜨지 않고, 그 확인을 기다리던 동작도 영영 일어나지 않는다.
  예·아니오는 `ConfirmModal`, 값을 받아야 하는 것은 `DeleteAccountModal`, 동작 실패 알림은
  `Screen` 위쪽 띠로 간다. `Alert.prompt`가 iOS 전용인 것도 같은 방향의 이유다.
  `Modal` 자체는 웹에도 제대로 구현돼 있으니 갈래를 나눌 필요는 없다.
- **`notice`와 `error`는 자리를 나눈다.** `error`는 목록을 못 받아온 것(화면 전체를 못 믿는다),
  `notice`는 방금 누른 동작만 실패한 것(목록은 멀쩡하다). 둘은 함께 뜰 수 있고 — 서버가 죽으면
  각각이 사실이다 — `notice`는 다음에 목록을 다시 받아오는 데 성공하면 걷힌다.
- **화면은 웹과 같은 묶음으로 나눈다.** 목록은 진행 중 / 완료(복습이 남은 것) /
  보관됨(접힘), 오늘은 오늘 수행 / 오늘 복습, 복습은 지남 / 오늘 / 예정이다.
  항목마다 카드를 두르지 않고 **한 카드(`Section`) 안에서 구분선으로만** 나눈다 —
  항목마다 테두리를 두르면 목록이 길어질수록 선이 시끄러워진다.
  웹 CSS의 조각들은 부품으로 옮겨 두었다: `Chip`(`.dl-tag` 무리) · `Checkbox` ·
  `PillButton`(`.pill-btn`) · `MoreMenu`(`⋯`) · `Section` · `Dot`/`StageBar`.
- **칩은 색으로 뜻을 나른다.** 지난 것은 danger, 오늘과 D-3 안쪽은 warn, 그 밖은 중립,
  예정된 복습·시간은 accent. 마감 D-3과 D-4는 **글자가 같고 색만 다르다** — 웹도 그렇다.
- 할 일이 가리키는 성격은 `category_id`다. 한때 앱 타입만 `category`로 적혀 있어서
  성격 칩이 영영 그려지지 않았다 — 서버(`formatTodo`)가 주는 이름을 그대로 쓴다.
  `completed`·`needs_review`도 boolean이 아니라 **0/1인 숫자다**(스키마가 Number다).
...
- **브라우저가 더 잘하는 것은 브라우저에 맡긴다.** 파일 이름 뒤에 `.web.tsx`를 붙이면
  Metro가 웹으로 묶을 때 그쪽을 먼저 고른다. 날짜·시각(`datetime-field`)과 진행률
  (`progress-field`)이 그렇게 갈려 있다 — 네이티브는
  `@react-native-community/datetimepicker`·`slider`를, 웹은 `<input type=date|time|range>`를
  쓴다. 그 두 라이브러리는 **웹을 지원하지 않으므로**, 갈라 두지 않으면 `npm run web`으로
  개발하는 동안 폼의 절반을 확인할 수 없다. 웹 번들에 그것들이 한 조각도 들어가지 않는지는
  `npx expo export --platform web` 뒤 번들을 훑어 확인할 수 있다.
- **글꼴도 웹과 같은 것을 쓴다.** 제목은 나눔명조(`fontFamily.display`), 본문은 웹과 같은
  글꼴 목록(`fontFamily.body`)이다. RN에는 **상속도 목록 폴백도 없어서** 글자를 그리는
  곳마다 지정해야 하고, 본문은 웹에서만 목록을 넘기고 네이티브는 시스템 기본에 맡긴다
  (iOS 기본이 마침 그 목록의 2순위다). 명조는 `_layout.tsx`에서 불러오는데
  **패키지 뿌리에서 import하면 안 쓰는 굵기까지 담긴다** — 한글 글꼴이라 하나에 3MB씩이다.
  파일을 직접 가리킬 것. 글꼴 파일이 800 굵기이므로 `fontWeight`를 함께 주지 않는다.
- **하단 탭바는 기본 것을 쓰지 않는다.** 웹은 아래에 떠 있는 반투명 알약이고
  react-navigation의 기본은 밑변에 붙은 띠라 아래쪽이 전혀 다르게 보였다.
  `Tabs`의 `tabBar`를 `FloatingTabBar`로 갈아 끼운다. **떠 있으므로 목록 위를 덮는다** —
  화면들은 `useTabBarSpace()`가 알려주는 만큼 아래를 비워야 한다(+ 버튼 위치도 여기서 온다).
- 웹의 화면 규칙은 여기서도 그대로다: 이모지 대신 선 아이콘(`@expo/vector-icons`),
  입력칸 16px 이상, 누르는 것 44px 이상, 드래그로만 되는 기능을 만들지 않는다.

### 아직 없는 것

오늘 화면의 타임라인 보기(계산은 `shared/timeline.js`에 이미 있고 그리는 부분만 없다),
알림, 비밀번호 변경, 성격 관리. 설정 화면에는 아직 계정에 관한 것만 있다.

오늘 화면의 **진행률은 읽기 전용이다.** 웹은 항목 줄에서 슬라이더로 그 자리에서 고치는데,
앱에서는 아직 폼(수정)에서만 고칠 수 있다.

아이폰 실기기에서 **한 번도 띄워보지 못했다.** App Store의 Expo Go가 SDK 54에 멈춰 있어서
(애플 심사 대기) SDK 57 프로젝트가 열리지 않는다. `eas go`로 우회하려면 Apple Developer
Program이 필요하다. 그래서 지금 확인은 `npm run web`으로만 한다 — 날짜 선택기의 iOS 시트,
키보드 회피, 노치 겹침은 실기기가 생기는 날 1순위로 볼 것.
